...
import { Instance as Script } from "./S2GBADialogFetcher.js";
...
if (Args.Filename != "" && Args.Outfolder != "") {
	...
	if (Script.IsGood()) {
		if (Args.Outfolder[Args.Outfolder.length - 1] != "/") { // Only do this, if the last character is not / inside that argument.
			...
			for (let LangIdx = 0x0; LangIdx < LangNames.length; LangIdx++) DenoHelper.WriteTextToFile(Script.Extract(LangIdx), (Args.Outfolder + "/" + LangNames[LangIdx] + "-Dialogs.txt"));
		}
